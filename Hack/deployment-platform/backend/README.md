...
Create a `.env` file from `.env.example` or export the following:
```bash
...
export DEPLOYMENT_VALIDATOR_ADDRESS='0xYourDeployedContractAddress'
export DEBUG='True'  # Set to 'False' in production
```

The backend loads `backend/.env` automatically and will also read the repo root `deployed-address.json` generated by the Hardhat deploy script if `DEPLOYMENT_VALIDATOR_ADDRESS` is not set explicitly.
