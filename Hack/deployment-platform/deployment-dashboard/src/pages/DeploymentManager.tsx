...
export default function DeploymentManager() {
  ...
  const [hiddenDeploymentIds, setHiddenDeploymentIds] = useState<string[]>([]);
  const { toast } = useToast();

  const HIDDEN_STORAGE_KEY = "starfall-hidden-deployments";

  const filterHiddenDeployments = (
    list: Deployment[],
    overrideHidden?: string[]
  ) => {
    const hiddenIds = overrideHidden ?? hiddenDeploymentIds;
    return list.filter((deployment) => !hiddenIds.includes(deployment.id));
  };

  const persistHiddenDeploymentIds = (ids: string[]) => {
    localStorage.setItem(HIDDEN_STORAGE_KEY, JSON.stringify(ids));
    setHiddenDeploymentIds(ids);
  };
  ...
  const loadDeployments = async () => {
    try {
      ...
      const hiddenIds: string[] = storedHidden ? JSON.parse(storedHidden) : [];
      setHiddenDeploymentIds(hiddenIds);
      ...
      if (historyResult.success) {
          const backendDeployments = historyResult.history
            .map(item => ({
              ...
              repoUrl: item.display_repo_url || item.repo_url || 'Unknown Repository',
          ...
          if (backendDeployments.length === 0) {
            localStorage.removeItem("starfall-deployments");
          } else {
            localStorage.setItem("starfall-deployments", JSON.stringify(backendDeployments));
          }

          setDeployments(filterHiddenDeployments(backendDeployments, hiddenIds));
      } else {
        // Fallback to local storage only
        setDeployments(filterHiddenDeployments(localDeployments, hiddenIds));
      }
    } catch (error) {
      ...
      if (stored) {
        ...
        setDeployments(filterHiddenDeployments(parsed));
      }
    }
  };
  ...
  const saveToStorage = (updatedDeployments: Deployment[]) => {
    ...
    setDeployments(filterHiddenDeployments(updatedDeployments));
  };
  ...
  const saveRepoUpdate = async (id: string) => {
    ...
    try {
      ...
      if (deployResult.status === 'deployed') {
        const newUrl = deployResult.deployment_url || `http://127.0.0.1:9000/`;
        const newBlockId = deployResult.block_hash || 'unknown';
        ...
        const revertedDeployments = deployments.map(d => 
          d.id === id ? { ...d, status: "active" as const } : d
        );
        setDeployments(revertedDeployments);
  ...
  const handleDelete = async (id: string) => {
    ...
    saveToStorage(updatedDeployments);
    const updatedHiddenIds = hiddenDeploymentIds.includes(id)
      ? hiddenDeploymentIds
      : [...hiddenDeploymentIds, id];
    persistHiddenDeploymentIds(updatedHiddenIds);
                      ...
                      <>
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-sm text-sky-400 font-mono break-all">
                            {getRepoName(deployment.repoUrl) || deployment.repoUrl}
                          </p>
                          <button
